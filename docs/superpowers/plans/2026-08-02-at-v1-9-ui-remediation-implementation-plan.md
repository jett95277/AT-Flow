...
- [ ] **Step 1: Write a failing SessionList interaction test**
...
- [ ] **Step 2: Run the targeted test and confirm it fails because session entries are not buttons**

Run: `npm.cmd test -- --run src/components/SessionList.test.tsx`

- [ ] **Step 3: Implement selectable and active session rows**

Use a button per session, expose phase/status/update time, and set
`aria-current={activeSessionId === session.id ? "true" : undefined}`.

- [ ] **Step 4: Add an AppShell test proving a selected session becomes the active state source**
...
- [ ] **Step 5: Implement `selectSession(session)` in AppShell**
...
- [ ] **Step 6: Run SessionList and AppShell tests**
...
- [ ] **Step 1: Write failing tests for task validation and initial CodeAgent labeling**
...
- [ ] **Step 2: Run RunControls tests and confirm the missing-input failure**

Run: `npm.cmd test -- --run src/components/RunControls.test.tsx`

- [ ] **Step 3: Implement the new-session form inside RunControls**
...
- [ ] **Step 4: Write an AppShell test for the POST payload**
...
- [ ] **Step 5: Replace the hard-coded task in AppShell**
...
- [ ] **Step 6: Run RunControls and AppShell tests**
...
- [ ] **Step 1: Write a failing AppShell test for typed API errors**
...
- [ ] **Step 2: Write a failing test proving a later successful action clears the error**

Trigger one rejected command, then a successful command, and assert the stale
error is removed.

- [ ] **Step 3: Run AppShell tests and confirm both failures**

Run: `npm.cmd test -- --run src/components/AppShell.test.tsx`

- [ ] **Step 4: Implement `toApiError` and explicit action lifecycle helpers**
...
- [ ] **Step 1: Write a failing workspace-tree test**
...
- [ ] **Step 2: Run the targeted test and confirm `shared/agents` is present**

Run: `python -m unittest tests.test_web_workspace_service tests.test_web_api`

- [ ] **Step 3: Filter only the root-level `agents` child under the shared alias**
...
- [ ] **Step 1: Add structural class assertions to AppShell tests**
...
- [ ] **Step 4: Check the workspace diff**
