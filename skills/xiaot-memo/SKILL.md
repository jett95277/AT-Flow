---
name: 'xiaot-memo'
description: '一句话记录到 AT 记忆。使用当用户说"记一下 / 记录 / 保存"并给出结论、约束或未决问题时。调用 at memory add 沉淀三字段。'
metadata:
  domain: 'memory'
  source: 'manual'
---
...
short 准入规则：**必须绑定 task id**。推不出 task 时先跑 `& $Xiaot.ATCommand memory view` 查现有 task，仍无则明确告知大哥"无法确定 task，未记录"，绝不硬编：

```powershell
& $Xiaot.ATCommand memory add memory://session/<stage>-<task>/short --conclusion "<文本>" [--constraint "<约束>"] [--unresolved "<未决>"] --task <task-id>
```
