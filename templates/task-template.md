...
> 创建专题时按此模板组织任务级记忆内容。
> 存储位置：`memory://task/<NNN>-<slug>/medium`（xiaot_memory 记忆层管理，物理文件 `.agent/memory/medium/task-<id>.md`）

## task-id 规则

`<3位序号>-<英文短横线slug>`，如 `001-demo`、`008-fix-beam`。
序号 = 现有最大序号 +1（查 `xiaot-memory memory view`），无则从 `001` 开始。
...
- 阶段完成：`xiaot-memory memory add memory://session/<阶段>-<task-id>/short --conclusion "<结论>" --task <task-id>` + `xiaot-memory memory checkpoint`

## 生命周期（v3.1 严格动词）

`candidate`（`xiaot-memory memory add`，short 必绑 task）→ `verified`（`xiaot-memory memory verify --evidence`）
